    ...
    @Override
        public void process(GFF3Record record) {
            ...
            if(m.find()) {
				...
				ref = ref.replace("locus:","");
				ref = ref.replace("gene:","");
            ...
            // For the following feature classes, check if the GFF3 attribute `affy_id` is defined.
            ...
            if(m.find()) {
                if(record.getAttributes().get("affy_id") != null){
                    String name = record.getAttributes().get("affy_id").iterator().next();
                    feature.setAttribute("affymetricsIdentifier", name);
                }
            }
